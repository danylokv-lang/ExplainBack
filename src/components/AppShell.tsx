...
import { InterfaceLanguageToggle } from "./InterfaceLanguageToggle";
...
export function AppShell({
  ...
  // Built from a serializable `locale` prop rather than received as a `t` prop:
  // the Dictionary's parameterized entries are functions, which cannot cross
  // the Server->Client boundary as props.
  const t = dictionary(locale);

  const NAV = [
    { href: "/app", label: t.nav.overview, icon: HomeIcon },
    { href: "/app/practice", label: t.nav.practice, icon: PencilIcon },
    { href: "/app/cards", label: t.nav.cards, icon: LayersIcon },
    ...
    <LocaleProvider locale={locale} t={t}>
      <div className="min-h-dvh bg-bg lg:grid lg:grid-cols-[264px_minmax(0,1fr)]">
        ...
        <aside className="border-b border-rule bg-surface lg:sticky lg:top-0 lg:flex lg:h-dvh lg:flex-col lg:border-b-0 lg:border-r">
          ...
          >
            <nav aria-label={t.nav.overview} className="mt-4 lg:mt-2">
            ...
            <div className="mt-8 border-t border-rule pt-5 lg:mt-auto">
              ...
              <div className="mt-3 flex items-center justify-between gap-2 px-2">
                <p className="label">{t.shell.interfaceLanguage}</p>
